...
const ReusablePriorityPage = ({ priority }: Props) => {
    ...
    const { data: tasks, isLoading, isError: isTaskError } = useGetTasksByUserQuery(userId || 0, { skip: userId === null });
    const isDarkMode = useAppSelector((state) => state.global.isDarkMode);
    const filtererTasks = tasks?.filter(
                    ...
                    : view === "list"
                        ? (
                            <div className='grid grid-cols-1 gap-4'>
                                {filtererTasks?.map((task: Task) => (
                                    <TaskCard key={task.id} task={task} />
                                ))}
                            </div>
                        ) : (
                            view === "table" && filtererTasks && (
