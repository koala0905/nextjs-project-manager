...
const BoardView = ({ id, setIsModalNewTaskOpen }: BoardProps) => {
    ...
    return (
        <DndProvider backend={HTML5Backend}>
            <div className='grid grid-cols-1 gpa-4 p-4 md:grid-cols-2 xl:grid-cols-4'>
    ...
    return (
        ...
        >
            <div className='mb-3 flex w-full'>
                <div
                    className={`w-2 !bg-[${statusColor[status]}] rounded-lg`}
                    style={{ backgroundColor: statusColor[status] }}
                />
                <div className='flex w-full item-center justify-between rounded-e-lg bg-white px-5 py-4 dark:bg-dark-secondary'>
                    <h3 className='flex items-center text-lg font-semibold dark:text-white'>
                        {status}{' '}
                        <span className='ml-2 inline-block rounded-full bg-gray-200 p-1 text-center text-sm leading-noon dark:bg-dark-tertiary'
        ...
        <div ref={(instance) => {
            ...
            <div className='p-4 md:p-6'>
                <div className='flex items-start justify-between'>
                    <div className='flex flex-1 flex-warp items-center gap-2'>
                        {task.priority && <PriorityTag priority={task.priority} />}
                        <div className='flex gap-2'>
                            {taskTagSplit.map((tag) => (
                                <div key={tag} className='rounded-full bg-blue-100 px-2 py-1 text-sm'>
                                    {' '}{tag}
                                </div>
                            ))}
                        </div>
